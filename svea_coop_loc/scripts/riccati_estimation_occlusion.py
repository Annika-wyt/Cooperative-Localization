...
from svea_thesis.srv import GetPosition, GetPositionResponse, SetPosition
...
class riccati_estimation_occlusion():
    def __init__(self):
        ...
        self.VisActPosePub = rospy.Publisher('~visualize/actualPose' + str(self.SUFFIX), VMA, queue_size=10)
        ...
        # rospy.Subscriber(cameraInfo_topic, CameraInfo, self.CameraInfoCallback)
        self.vehicleSubscription(communication)
        rospy.Subscriber("/qualisys/" + self.SVEANAME + "/pose", PoseStamped, self.ThisqualisysCallback)
        

        # Service
        self.pose_service = rospy.Service("/update_pose_service", SetPosition, self.update_pose_callback)
        ...
        twist_topic = rospy.get_param('~twist_topic', '/qualisys/' + self.SVEANAME + '/velocity')
        twist_sub = Subscriber(twist_topic, TwistStamped)
...
############################################# VISUALIZATION ##############################################
    def visualizePose(self, ns, transformed_direction):
        VisArrMsg = VMA()
        VisMarker = VM()
        VisMarker.header.stamp = self.timeStamp
        VisMarker.header.frame_id = self.MAPFRAME
        ...
        VisMarker.scale = Vector3(*[1.0, 0.3, 0.3])
        VisMarker.color = ColorRGBA(*[355.0/450.0, 0.0/450.0, 355.0/450.0, 1.0]) if ns == "RiccatiPose" else ColorRGBA(*[10.0/450.0, 10.0/450.0, 10.0/450.0, 0.9]) 
        VisMarker.points = [Point(*[0.5,0.5,2.0])]
        VisArrMsg.markers.append(VisMarker)
        if ns == "RiccatiPose":
            self.VisPosePub.publish(VisArrMsg)
        elif ns == "ActualPose":
            self.VisActPosePub.publish(VisArrMsg)
        ...
        # elif ns == "ActualPose":
            # self.ActpointPub.publish(pointMsg)
...
############################################# TRANSFORM ##############################################
    ...
    def sendTransPose(self):
        ...
        cammsg.pose.pose.position = Vector3(*self.estpose)
        ...
        AllTransforms.append(msg)
        self.sbr.sendTransform(AllTransforms)   
        self.visualizePose("RiccatiPose", transformed_direction) 
...
############################################# CALLBACKS ##############################################
    ...
    def ThisqualisysCallback(self, msg):
        self.visualizePose("ActualPose", msg)
        ...
        if self.DoneInitialization >= 3:
            ...
            for bearing in landmarkMsg.bearing:
                if bearing.id in self.hdmap:
                    ...
                    type = 1
                    if self.communicationMethod == "NATS":
                        try:
                            ######################################## with NATS SERVICE ########################################
                    ...
                    elif self.communicationMethod == "qualisys":
                        try:
                            ######################################## with qualisys ########################################
                            whichSvea = self.vehicleAgents[bearing.id]["sveaId"]
                            self.landmarks[bearing.id] = {'type': type, 
            ...
            linear_velocity = np.array([twistMsg.twist.linear.x, twistMsg.twist.linear.y, twistMsg.twist.linear.z])
            # linear_velocity = np.dot(self.transBaseCamVelocityRot, linear_velocity)    
            angular_velocity = np.array([twistMsg.twist.angular.x, twistMsg.twist.angular.y, twistMsg.twist.angular.z])
...
############################################# SERVERS ##############################################
    ...
    def update_pose_callback(self, req):
        self.DoneInitialization = 0
       

        updatepose_x = max(-1, min(req.pose_x, 5)) 
        updatepose_y = max(-5, min(req.pose_y, 5)) 
        updatepose_z = max(-5, min(req.pose_z, 5)) 
        self.estpose = np.array([updatepose_x, updatepose_y, updatepose_z], dtype=np.float64)
        rospy.loginfo(f"Received new pose update: {self.estpose}")
        AllTransforms = []
        msg = TransformStamped()
        msg.header.stamp = self.timeStamp
        msg.header.frame_id = self.MAPFRAME
        msg.child_frame_id = self.baseFrame + self.SUFFIX
        msg.transform.translation.x = updatepose_x
        msg.transform.translation.y = updatepose_y
        msg.transform.translation.z = updatepose_z
