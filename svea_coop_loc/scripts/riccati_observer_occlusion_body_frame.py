...
class riccati_observer_occlusion():
    ...
    def set_init(self, ori, pose):
        ...
        print(f"""/riccati_observer_occlusion/set_init
        ##########################################################################              
            Initial Estimation
    ...
    def function_A(self):
        ...
        Output = 6x6 matrix
        '''
        A11 = -self.function_S(self.angvel)
        A12 = np.zeros((3,3))
        A21 = np.zeros((3,3))
        A22 = -self.function_S(self.angvel)
    ...
    def function_C(self, input_R_hat_i):
        '''
        ...
        for landmark_id, landmark_info in self.landmarks.items():
            ...
            # S(hat_R_i^T hat_p)
            first = self.function_S(np.matmul(np.transpose(input_R_hat_i), landmark_info["position"]))
            # -pi_g S(hat_R_i^T hat_p)
            first = -np.matmul(pi_g, first)

            C_landmark = np.hstack((first, pi_g))
            if j == 0:
                output_C = C_landmark
            else:
    ...
    def observer_equations(self, input_R_hat_i, input_p_hat_i, input_P):
        ...
        first_lower = -np.matmul(self.function_S(self.angvel), input_p_hat_i) + self.linvel
        # first_lower = -np.cross(self.angvel, input_p_hat_i) + self.linvel
        ...
        for landmark_id, landmark_info in self.landmarks.items():
            ...
            common = np.matmul(pi_g, (input_p_hat_i -np.matmul(np.transpose(input_R_hat_i), landmark_info["position"])))
            # S(R_hat_i.T * z_j)
            S_rp = self.function_S(np.matmul(np.transpose(input_R_hat_i),  landmark_info["position"]))
            # q * S() * pi_g * R_hat_i.T * p_hat_ij
            upper = self.q * np.matmul(S_rp, common)
            # q * pi_g * R_hat_i.T * p_hat_ij
            lower = self.q * common
    ...
    def dynamics(self, t, y):
        ...
        ####################################
        ############# Observer #############
        output_omega_hat_p_hat_dot = self.observer_equations(input_R_hat, input_p_bar_hat, input_P)
        ############# Observer #############
        ####################################
        ...
        p_bar_hat_dot = output_omega_hat_p_hat_dot[3:]
        ####################################
        ############ Quaternion ############
        ...
        return np.concatenate((output_qua_hat_flat, p_bar_hat_dot.flatten(), output_P_dot.flatten()))
        ########### Quaternion ############
        ####################################
